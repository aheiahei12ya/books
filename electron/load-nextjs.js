...
const adjustRenderer = (directory) => {
  ...
  protocol.interceptFileProtocol('file', (request, callback) => {
    ...
    for (const prefix of paths) {
      ...
      newPath = join(directory, 'dist', newPath)
      path = newPath
    }
...
module.exports = async (directories, port, dev = false) => {
  if (!directories) {
  ...
  await runServer(directories.development, port, dev)
}
